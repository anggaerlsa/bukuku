{{--
    ...
    Parameter:
      $comment  — komentarnya
      $storeUrl   — action form balas (sama dengan form kirim di surface ini)
      $ownerId    — user_id pemilik novel, untuk lencana "Penulis"
      $canComment — apakah pengguna ini boleh memuat komentar/balasan
      $isReply    — true bila ini balasan
--}}
@php($isReply = $isReply ?? false)
@php($canComment = $canComment ?? false)
...
<div class="flex gap-3" x-data="{ sunting: false, balas: false }">
    ...
    <div class="min-w-0 flex-1">
        ...
        {{-- Form balas (hanya di komentar utama), memakai storeUrl surface ini --}}
        @unless ($isReply)
            @if ($canComment)
                <form method="POST" action="{{ $storeUrl }}" x-show="balas" x-cloak class="mt-3" @submit="balas = false">
                    @csrf
